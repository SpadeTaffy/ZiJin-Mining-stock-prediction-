...
OUT=Path(os.environ.get('MLP15_PX_OUT',str(ROOT/'results/variable_focus_2017_15')));OUT.mkdir(parents=True,exist_ok=True)
...
idx=levels.index[levels.index.year==2017];assert len(idx)==267
...
updates=updates[(updates.Year==2017)&(updates.Method=='Features_and_triggers')].copy()
...
signals=pd.read_csv(UPDATE_SOURCE/'2017'/'expanded_signals.csv',parse_dates=['Trigger_date','Effective_date'])
...
for name,group in groups.items():
    ...
    buttons.append(dict(label=name,method='update',args=[{'visible':visible},{'title.text':f'2017｜股价、预测与{name}：逐变量细看','yaxis2.autorange':True,'yaxis3.range':[-lim,lim]}]))
fig_focus.update_layout(template='plotly_white',title=dict(text='2017｜股价、预测与黄金：逐变量细看',y=.98,x=.05),height=1100,hovermode='x unified',font=dict(family='Arial, PingFang SC, Microsoft YaHei, sans-serif'),margin=dict(l=80,r=220,t=155,b=80),legend=dict(x=1.03,y=1,title='点击切换曲线'),updatemenus=[dict(type='dropdown',active=list(names.values()).index(default),buttons=buttons,x=0,y=1.075,xanchor='left',yanchor='top')])
...
fig_focus.write_html(OUT/'2017_variable_focus.html',include_plotlyjs=True,config=config);fig_focus.write_json(OUT/'focus_figure.json')
