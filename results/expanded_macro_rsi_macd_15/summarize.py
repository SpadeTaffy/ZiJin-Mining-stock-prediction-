...
for offset,col,label,color in [(-.2,'Baseline','基准：RSI＋MACD＋原多条件触发','#78909c'),(.2,'Expanded','新增三项偏离特征＋扩展触发','#009e73')]:
 bars=ax.bar(pos+offset,t[col],width=.38,label=label,color=color);ax.bar_label(bars,fmt='%.2f',padding=3,fontsize=9)
ax.set(xticks=pos,xticklabels=[str(i) if i!=2026 else '2026*' for i in t.index],ylabel='MAPE（%，越低越好）',title='2015—2026年：新增宏观偏离特征及触发条件的对比',ylim=(0,t[['Baseline','Expanded']].to_numpy().max()*1.2));ax.legend();ax.grid(axis='y',alpha=.2);ax.set_axisbelow(True)
...
bars=ax.bar(['基准模型','新增特征＋扩展触发'],vals,color=['#78909c','#009e73'],width=.55);ax.bar_label(bars,fmt='%.4f%%',padding=5);ax.set(ylabel='年度等权平均MAPE（%）',title='2015—2026年平均表现（2026为部分年度）',ylim=(0,vals.max()*1.22));fig.savefig(OUT/'average_mape.png',dpi=160);plt.close(fig)
print(t.round(5).to_string());print(summary.round(6).to_string(index=False))
(OUT/'report.md').write_text('# 新增宏观偏离特征与触发条件对比\n\n'+t.round(5).to_markdown()+'\n\n'+summary.round(6).to_markdown(index=False)+'\n\nMAPE差值为新方案减基准，负值表示改善。年度等权平均为各年MAPE的算术平均；2026仅截至7月23日。此次同时改变特征及更新频率，结果不能分别归因于单一改动。',encoding='utf-8')
