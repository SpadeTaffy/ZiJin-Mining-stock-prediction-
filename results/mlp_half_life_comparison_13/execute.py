...
os.environ['MLP13_GRID_OUT']=str(Path('outputs/mlp_half_life_comparison_13').resolve())
os.environ['MPLCONFIGDIR']='/private/tmp/mlp11-matplotlib'
p=Path('outputs/13_加权MSE的训练.ipynb')
n=nbformat.read(p,as_version=4); shell=InteractiveShell.instance(); scope={}; count=0
for cell in n.cells[int(Path("outputs/mlp_half_life_comparison_13/start_cell.txt").read_text()):]:
