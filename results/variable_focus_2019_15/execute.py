...
os.environ['MLP15_PX_OUT']=str(Path('outputs/variable_focus_2019_15').resolve())
os.environ['MPLCONFIGDIR']='/private/tmp/mlp11-matplotlib'
p=Path('outputs/15_分析每一年数据,细化更新条件.ipynb')
n=nbformat.read(p,as_version=4); shell=InteractiveShell.instance(); scope={}; count=0
for cell in n.cells[int(Path("outputs/variable_focus_2019_15/start_cell.txt").read_text()):]:
...
print('2019 focus chart complete.')
