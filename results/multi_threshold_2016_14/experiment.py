...
OUT=Path(os.environ.get('MLP14_MULTI_OUT',str(Path(__file__).resolve().parent)))
...
def experiment():
    ...
    te=x.index[x.index.year==2016];assert len(te)==268
        ...
        for col,thresholds in rules.items():
            for threshold in thresholds:
                ...
                for day in te[changed.loc[te].to_numpy()]:
                    ...
                    if pd.notna(effective) and effective.year==2016:
    ...
    old_updates=pd.read_csv(prior/'2016/updates.csv',parse_dates=['Effective_date']);old_updates=old_updates[old_updates.Method=='Crossing_H60']
    ...
    for date in new_schedule.Effective_date:
        anchor=pd.Timestamp('2016-01-01') if date==te.min() else date
        cache=prior/'2016'/f'selection_{anchor:%Y-%m-%d}.json'
        if cache.exists():
    ...
    print(f'Original updates {len(old_schedule)-1}; expanded {len(new_schedule)-1}; new selection dates {new_selections}',flush=True)
    ...
    for j,date in enumerate(new_schedule.Effective_date):
        ...
        if (j+1)%25==0:print(f'Refit {j+1}/{len(new_schedule)} dates',flush=True)
    ...
    for method,(sched,expanded) in methods.items():
        ...
        for j,row in enumerate(sched.itertuples(index=False)):
            date=row.Effective_date;stop=sched.iloc[j+1].Effective_date if j+1<len(sched) else pd.Timestamp('2017-01-01')
    ...
    oldpred=pd.read_csv(prior/'2016/predictions.csv',parse_dates=['Date'])
