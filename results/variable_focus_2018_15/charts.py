...
OUT=Path(os.environ.get('MLP15_PX_OUT',str(ROOT/'results/variable_focus_2018_15')));OUT.mkdir(parents=True,exist_ok=True)
...
idx=levels.index[levels.index.year==2018];assert len(idx)==269
...
PRED_SOURCE=OUT.parent/'rsi_macd_annual_14'
if not (PRED_SOURCE/'predictions.csv').exists():PRED_SOURCE=ROOT/'results/rsi_macd_annual_14'
UPDATE_SOURCE=OUT.parent/'multi_threshold_annual_14'
if not (UPDATE_SOURCE/'updates.csv').exists():UPDATE_SOURCE=ROOT/'results/multi_threshold_annual_14'
...
updates=updates[(updates.Year==2018)&(updates.Method=='Features_and_triggers')].copy()
...
indexed['最新模型预测（RSI＋MACD）']=100*pred.Both/base['股价']
assert np.allclose(indexed.drop(columns='最新模型预测（RSI＋MACD）').iloc[0],100)
LABELS=['相邻记录变化','相对MA5偏离','相对MA20偏离']
colors={'股价':'#222222','最新模型预测（RSI＋MACD）':'#009E73','A500指数':'#6F4E7C','美元指数':'#CC6677','中国10年期利率':'#44AA99','美国10年期利率':'#882255','布伦特原油':'#AA7733','铜':'#0072B2','黄金':'#D59400',LABELS[0]:'#8B9199',LABELS[1]:'#009E73',LABELS[2]:'#CC79A7'}
...
rules={'黄金':[3],'铜':[3,8],'A500指数':[5,8],'美国10年期利率':[10,20]}
variable_names={'gold_vs_ma20':'黄金','copper_vs_ma20':'铜','a500_vs_ma20':'A500指数','us10y_vs_ma20':'美国10年期利率'}
signals=pd.read_csv(UPDATE_SOURCE/'2018'/'expanded_signals.csv',parse_dates=['Trigger_date','Effective_date'])
...
for name in ['股价','最新模型预测（RSI＋MACD）']:
...
for name,group in groups.items():
    ...
    buttons.append(dict(label=name,method='update',args=[{'visible':visible},{'title.text':f'2018｜股价、预测与{name}：逐变量细看','yaxis2.autorange':True,'yaxis3.range':[-lim,lim]}]))
fig_focus.update_layout(template='plotly_white',title=dict(text='2018｜股价、预测与黄金：逐变量细看',y=.98,x=.05),height=1100,hovermode='x unified',font=dict(family='Arial, PingFang SC, Microsoft YaHei, sans-serif'),margin=dict(l=80,r=220,t=155,b=80),legend=dict(x=1.03,y=1,title='点击切换曲线'),updatemenus=[dict(type='dropdown',active=list(names.values()).index(default),buttons=buttons,x=0,y=1.075,xanchor='left',yanchor='top')])
...
fig_focus.write_html(OUT/'2018_variable_focus.html',include_plotlyjs=True,config=config);fig_focus.write_json(OUT/'focus_figure.json')
chart=levels.loc[idx].copy();chart['最新模型预测（RSI＋MACD）']=pred.Both
for name in levels:
...
print(f"2018年：{len(idx)}条记录，7项变量可切换，{len(events)}次触发更新。")
display(fig_focus)
